...
/// Mirrors BackendManagerController.PingProbe — TCP connect to probe.host:probe.port,
/// send "ping", measure RTT to first response. Background sweeps return per-probe results
/// via OnResult on whatever thread the TCP callback fires from; the UI marshals to main thread.
///
/// Successor to BeaconPinger (Edgegap Beacons -> Probes migration).
public static class ProbePinger
{
    ...
    // dead regions. 8 in flight is enough to finish a 20-probe sweep in
    ...
    /// Kick off a one-shot sweep across the provided probes. No-op if a sweep
    /// is already running or we're still in cooldown.
    public static bool TryStartSweep(IReadOnlyList<Probe> probes)
    {
        ...
        Task.Run(() => SweepCore(probes));
        return true;
    }

    private static void SweepCore(IReadOnlyList<Probe> probes)
    {
        ...
        try
        {
            ...
            foreach (var p in probes)
            {
                ...
                tasks.Add(Task.Run(() =>
                {
                    try
                    {
                        int? rtt = PingOne(probe);
                        try { OnResult?.Invoke(new PingResult { ProbeId = probe.id, RttMs = rtt }); }
    ...
    private static int? PingOne(Probe probe)
    {
        try
        {
            var endPoint = new EndPoint(probe.host, probe.port);
        ...
        {
            Plugin.LogDebug($"Ping to {probe?.host}:{probe?.port} failed: {e.Message}");
            return null;
        }
    }
}
