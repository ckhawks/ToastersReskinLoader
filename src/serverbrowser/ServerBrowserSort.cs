...
internal static class ServerBrowserSort
{
    ...
    private static bool FavoritesEnabled =>
        Settings.Current?.enableServerFavorites ?? false;
    private static bool BlocksEnabled =>
        Settings.Current?.enableServerBlocks ?? false;
    ...
    private static bool SavedPasswordsEnabled =>
        Settings.Current?.enableSavedServerPasswords ?? false;
    ...
    private static bool SortTweaksEnabled =>
        Settings.Current?.enableServerBrowserSortTweaks ?? true;
    ...
    private static readonly ServerKeyStore _favorites =
        new ServerKeyStore(() => Settings.Current?.favoriteServers);
    private static readonly ServerKeyStore _blocked =
        new ServerKeyStore(() => Settings.Current?.blockedServers);
    ...
    {
        if (_favorites.Remove(key)) Settings.Save();
    }

    public static void RemoveAllFavorites()
    {
        if (_favorites.Clear()) Settings.Save();
    }
    ...
    {
        if (_blocked.Remove(key)) Settings.Save();
    }

    public static void RemoveAllBlocks()
    {
        if (_blocked.Clear()) Settings.Save();
    }
    ...
    private static class StyleServer_AddBadges_Postfix
    {
        private static void Postfix(UIServerBrowser __instance, EndPoint endPoint)
        {
            ...
            try
            {
                ...
                {
                    var s = Settings.Current?.favoriteServers;
                ...
                bool willAutoFill = (Settings.Current?.enableSavedServerPasswords ?? false)
    ...
    {
        var store = Settings.Current?.savedServerPasswords;
